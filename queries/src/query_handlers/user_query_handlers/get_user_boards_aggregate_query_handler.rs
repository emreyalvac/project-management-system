    ...
    async fn get(&self) -> Result<BoardUserAggregate, BoardUserAggregate> {
        ...
        match connection {
            Ok(client) => {
                ...
                let query_4 = doc! {"$group" => {"_id": "$_id", "boards" => {"$addToSet" => {"board_id": "$boards.board_id", "board_name": "$boards.board_name", "board_manager_user_id": "$boards.board_manager_user_id"}}}};
